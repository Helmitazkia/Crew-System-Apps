...
class Report extends CI_Controller {
	...
	function getDataCVSuntechno($idPerson = "", $company = "")
	{
		...
		$sql = "SELECT 
					...
					C.NmNegara, 
					D.NmKota AS pKota, 
					DATE_FORMAT(FROM_DAYS(DATEDIFF(CURRENT_DATE, A.dob)), '%y') AS age,
					COUNT(E.signoffdt) AS total_contracts,
					SUM(TIMESTAMPDIFF(YEAR, E.signondt, E.signoffdt)) AS total_years
				FROM mstpersonal A
				LEFT JOIN tblkota B ON A.pob = B.KdKota
				LEFT JOIN tblnegara C ON A.nationalid = C.KdNegara
				LEFT JOIN tblkota D ON A.pcity = D.KdKota
				LEFT JOIN tblcontract E ON A.idperson = E.idperson 
					AND E.kdcmprec = '005' 
					AND E.signoffdt IS NOT NULL
				WHERE A.deletests = '0' 
		...
		if(count($rsl) > 0)
		{
			$photo = $rsl[0]->pic ? "<img src=\"".base_url('imgProfile/'.$rsl[0]->pic)."\" style=\"width:90px;height:120px;\">" : "";
			...
			$dataOut['yearyOpSun'] = $rsl[0]->total_contracts. " years";
			$dataOut['yearyRankSun'] = $rsl[0]->total_years. " years";
