...
class Report extends CI_Controller {
	...
	function searchDataPipeline() { 
		$search = $this->input->get('search');
		$page = $this->input->get('page');
		$this->getDataPipelineCrew($search, $page);
	}
	...
	function getDataPipelineCrew($search = "", $page = 1)
	{
		$page = isset($_GET['page']) && is_numeric($_GET['page']) ? intval($_GET['page']) : 1;
		$limit = 10;	
		$offset = ($page - 1) * $limit;
		...
		$sqlTotal = "SELECT COUNT(*) as total FROM new_applicant 
				WHERE deletests = '0' AND st_data IN (2,3) 
				AND (position_applied LIKE '%$search%' OR fullname LIKE '%$search%')";
			...
			<div style='font-family: Arial, sans-serif; background-color: #f4f4f4; padding: 30px;'>
				<div style='max-width: 600px; margin: auto; background-color: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.05); border: 1px solid #e0e0e0;'>
					...
					<div style='background-color: #003366; padding: 20px; text-align: center;'>
						<img src='https://andhika.com/wp-content/uploads/2022/10/cropped-Logo-Andhika-02-2048x476.png' alt='PT Andhika Lines' style='max-width: 180px;'>
					</div>
