...
class Personal extends CI_Controller {
	...
	function getData($searchNya = "",$pageNya = "")
	{
		...
		{
			$whereNya .= " AND A.idperson LIKE '%".$pageNya."%' ";
		}
	...
	function getDataProses()
	{
		...
		if($type == "editProses")
		{
			...
			if(count($rsl) > 0)
			{
				$fullName = preg_replace('/\s+/', ' ', trim($rsl[0]->fname." ".$rsl[0]->mname." ".$rsl[0]->lname));
				$tempRsl = array(   "txtoldcrewID" => $rsl[0]->oldcrewid,
									"txtoldcontno" => $rsl[0]->oldcontno,
									"txtFname" => preg_replace('/\s+/', ' ', trim($rsl[0]->fname)),
									"txtMname" => preg_replace('/\s+/', ' ', trim($rsl[0]->mname)),
									...
									"txtPercentageHomeSalary" => $rsl[0]->percentage_home_salary,
									"txtBankNameBoat" => $rsl[0]->bank_name_boat,
									"txtAccNoBoat" => $rsl[0]->norek_boat,
									"txtAccountNameBoat" => $rsl[0]->norek_name_boat,
									"txtPercentageBoardSalary" => $rsl[0]->percentage_board_salary,
	...
	function saveDataPersonal()
	{
		...
		$dataIns['kodepelaut'] = $data['seafarerCode'];
		$dataIns['fname'] = preg_replace('/\s+/', ' ', trim($data['fname']));
		$dataIns['mname'] = preg_replace('/\s+/', ' ', trim($data['mname']));
		$dataIns['lname'] = preg_replace('/\s+/', ' ', trim($data['lname']));
		...
		$dataIns['taxstsid'] = $data['taxStatus'];
		...
		$dataIns['percentage_home_salary'] = $data['txtPercentageHomeSalary'];
		$dataIns['bank_name_boat'] = $data['txtBankNameBoat'];
		$dataIns['norek_boat'] = $data['txtAccNoBoat'];
		$dataIns['norek_name_boat'] = $data['txtAccountNameBoat'];
		$dataIns['percentage_board_salary'] = $data['txtPercentageBoardSalary'];
		...
		$dataIns['oldcontno'] = $data['txtoldcontno'];
		$dataIns['oldcrewid'] = $data['txtoldcrewID'];
	...
	function getDataNewApplicent($searchNya = "")
	{
		...
		$sql = "SELECT A.idperson,
                ...
                A.applyfor, A.gender, A.religion, A.dob,
                B.NmKota,
                CASE WHEN A.inAktif = '0' THEN 'Aktif' WHEN A.inAktif = '1' THEN 'Non Aktif' END AS inAktif,
                CASE WHEN A.lower_rank = 0 THEN 'No' WHEN A.lower_rank =1 THEN 'Yes' END AS lowerrank,
	...
	function login()
	{
		...
		$sql = " SELECT * FROM login WHERE status = '0' AND userName = '".$user."' AND userPass = '".$pass."' ";
		$cekLogin = $this->MCrewscv->getDataQueryLogin($sql,$user,$pass);
	...
	function loginCrew() 
	{
		...
		$sql = " SELECT * FROM crew_login WHERE sts_delete = '0' AND userName = '".$user."' AND password = '".$pass."' ";
		$cekLogin = $this->MCrewscv->getDataQueryLogin($sql,$user,$pass);
