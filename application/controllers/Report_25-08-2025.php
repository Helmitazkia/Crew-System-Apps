...
class Report extends CI_Controller {
	...
	function getDataApplicantPositionSummary()
	{
		...
		$sql = "
			...
			AND st_qualify = 'N' AND st_qualify2 = 'N' AND YEAR(submit_cv) = '".$yearnow."'
	...
	function getDataApplicantPositionSummaryTalentPool()
	{
		$sql = "
			SELECT 
				...
				COUNT(*) as total
			FROM new_applicant
			WHERE deletests = 0 AND position_applied IS NOT NULL
			GROUP BY position_applied
		";
		...
		foreach ($positions as $pos) {
			$position = $pos->position_applied;

			$sqlDetail = "
				SELECT
					SUM(CASE WHEN st_data = 2 THEN 1 ELSE 0 END) AS not_position,
					SUM(CASE WHEN st_data = 3 THEN 1 ELSE 0 END) AS not_qualified,
					SUM(CASE WHEN st_data = 4 THEN 1 ELSE 0 END) AS not_reff,
					SUM(CASE WHEN st_data = 5 THEN 1 ELSE 0 END) AS interview,
					SUM(CASE WHEN st_data = 0 AND (st_qualify = 'Y') THEN 1 ELSE 0 END) AS qualified
				FROM new_applicant
				WHERE deletests = 0 AND position_applied = '".$position."'
			";

			$detail = $this->MCrewscv->getDataQuery($sqlDetail, array($position));
			...
			$result[] = array(
				...
				'not_qualified' => (int) $row->not_qualified,
				'not_reff' => (int) $row->not_reff,
	...
	function getApplicantDataByDate()
	{
		...
		if (!$startDate || !$endDate) {
			echo json_encode(array('newApplicants' => array(), 'talentPool' => array()));
			return;
		}
		...
		$sqlNew = "
			...
			GROUP BY position_applied
		";
		$dataNew = $this->MCrewscv->getDataQuery($sqlNew);
		...
		$sqlTalent = "
			...
			GROUP BY position_applied
		";
		$dataTalent = $this->MCrewscv->getDataQuery($sqlTalent);

		$resultTalent = array();
		foreach ($dataTalent as $row) {
			$resultTalent[] = array(
				'name' => $row->position_applied,
				'y' => (int)$row->total
			);
	...
	function searchDataQualifiedCrew()
	{
		$search = $this->input->get('search');
		$page = $this->input->get('page');
		$this->getQualifiedCrew($search, $page);
	}
	...
	function setNotQualifiedCrewLayer1()
	{
		...
		$certificates = $this->input->post('missing_certificates');
		...
		if ($id) {
			...
			$data = array(
				'st_data' => 3,
				'reason_not_qualified_layer1' => $reason,
				'adduserdate_notQualify' => $fullNameUser . "#" . $date,
			);
			$where = array('id' => $id);
			$this->MCrewscv->updateData($where, $data, 'new_applicant');

			$this->sendNotQualifiedNotificationLayer1($applicant->email, $applicant->fullname, $reason, $certificates);
	...
	function sendNotQualifiedNotificationLayer1($recipientEmail, $fullName, $reason, $certificates = "")
	{
	...
	function getQualifiedCrew($search = "", $page = 1)
	{
		$dataContext = new DataContext();
		$page = isset($_GET['page']) && is_numeric($_GET['page']) ? intval($_GET['page']) : 1;
		$limit = 10;
		...
		$sqlTotal = "SELECT COUNT(*) as total FROM new_applicant 
			WHERE deletests = '0' AND st_qualify = 'Y' AND st_data = '0'  AND st_qualify2 = 'N'
		...
		{ 
			...
			$trListQualifiedCrew .= "<td class='position-applied'>$val->position_applied</td>";
	...
	function getDataPipelineCrew($search = "", $page = 1, $gender = "")
	{
		...
		foreach ($rsl as $val) {
			...
			$label = '';
			if ($val->st_data == 2 && !empty($val->id)) {
				$btnAct .= "<button class=\"btn btn-success btn-xs btn-block\" style=\"margin-top:5px;\" onclick=\"positionAvailableCrew('".$val->id."', '<b><i>:: ".$val->fullname." ::</i></b>');\">Position Available</button>";
				$label = "<span class='badge' style='font-size: 11px; display: inline-block; margin-top: 4px; color:'>No Position</span>";
			} elseif ($val->st_data == 3) {
				$label = "<span class='badge badge-danger' style='font-size: 11px; display: inline-block; margin-top: 4px;'>Not Qualified</span>";
				$btnAct .= "<button class=\"btn btn-success btn-xs btn-block\" style=\"margin-top:5px;\" onclick=\"positionAvailableCrew('".$val->id."', '<b><i>:: ".$val->fullname." ::</i></b>');\">Position Available</button>";
			} elseif ($val->st_data == 4) {
				$label = "<span class='badge badge-success' style='font-size: 11px; display: inline-block; margin-top: 4px;'>No Reff</span>";
			}

			$trListDataDraftCrew .= "<tr id='row_$val->id'>";
			$trListDataDraftCrew .= "<td class='text-center'>$no</td>";
			$trListDataDraftCrew .= "<td class='email'>$val->email</td>";
			$trListDataDraftCrew .= "<td class='fullname'>$val->fullname<br>$label</td>";
			$trListDataDraftCrew .= "<td>$val->born_place</td>";
			$trListDataDraftCrew .= "<td>".$dataContext->convertReturnName($val->born_date)."</td>";
			...
			$trListDataDraftCrew .= "<td class='pengalaman-jenis-kapal'>$val->pengalaman_jeniskapal</td>";
			...
			$trListDataDraftCrew .= "<td>".$dataContext->convertReturnNameWithTime($val->submit_cv)."</td>";
	...
	function getDataInterviewCrew($search = "", $page = 1)
	{
		...
		foreach ($rsl as $val)
		{
			...
			$btnAct .= "<button class=\"btn btn-success btn-xs btn-block\" style=\"margin-top:5px;\" onclick=\"pickUpDataApplicant('".$val->id."');\"><i class='fas fa-truck-pickup' style='font-size:15px'></i> Qualified</button>";
			
			$btnAct .= "<button class=\"btn btn-danger btn-xs btn-block\" style=\"margin-top:5px;\" onclick=\"notReffCrew('".$val->id."', '<b><i>:: ".$val->fullname." ::</i></b>');\">
				<i class=\"fas fa-user-check\"></i> Not Qualified
			</button>";
	...
	function saveDataNewApplicent() {
		...
		if (!empty($check)) {
			$existingIdPerson = $check[0]->idperson;
			...
			$this->db->where('idperson', $existingIdPerson);
			$this->db->update('mstpersonal', $updateData);

			$loginCheckSql = "SELECT * FROM crew_login WHERE idperson = '".$existingIdPerson."' AND sts_delete = 0";
			$loginCheck = $this->MCrewscv->getDataQuery($loginCheckSql);
			...
			echo json_encode(array(
				'success'  => true,
				'message'  => 'Data sudah ada, idperson lama digunakan dan data diperbarui.',
				'idperson' => $existingIdPerson
			));
			return;
		}
		...
		echo json_encode(array(
			'success'   => true,
			'message'   => 'Data baru berhasil dimasukkan ke data personal dan akun login dibuat.',
			'idperson'  => $newIdPerson,
			'username'  => $generatedUsername,
	...
	function sendPickUpNotification($recipientEmail, $fullName, $username, $password)
	{
		...
		try {
			...
			$mail->isHTML(true);
			$mail->Subject = '🎉 Congratulations! Anda Lolos Interview - PT Andhika Lines 🎉';

			$mail->AltBody = "Yth. Bapak/Ibu $fullName,\n\n"
				. "🎉 Selamat! Anda telah dinyatakan lolos tahapan interview 🚀\n\n"
				. "Kami dengan senang hati menyampaikan bahwa Anda berhak melanjutkan ke proses berikutnya di PT Andhika Lines 🚢\n\n"
			...
			$mail->Body = "
				<div style='font-family: Arial, sans-serif; background-color: #f5f5f5; padding: 20px;'>
					...
					<div style='max-width: 600px; margin: auto; background: #ffffff; padding: 30px; border-radius: 8px; border: 1px solid #ddd;'>
						...
						<p style='font-size: 14px; color: #333; line-height: 1.6;'>
							Selamat! Anda telah <strong>lolos tahapan interview</strong> dan berhak melanjutkan ke proses berikutnya 
							bersama <strong>PT Andhika Lines</strong> 
						</p>

						<p style='font-size: 14px; color: #333; line-height: 1.6;'> Tim Crewing <strong>PT Andhika Lines</strong> akan menghubungi Anda untuk tahapan selanjutnya apabila diperlukan. Mohon pastikan kontak Anda tetap aktif untuk mempermudah komunikasi. </p>

						<p style='font-size: 14px; color: #333; line-height: 1.6;'>Mohon siapkan dokumen-dokumen yang telah diberi tahu oleh Tim Crewing.</p>
