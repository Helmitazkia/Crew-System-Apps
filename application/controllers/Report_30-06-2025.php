...
class Report extends CI_Controller {
	...
	function getDataApplicantPositionSummaryDraft()
	{
		$sql = "
			SELECT position_applied, COUNT(*) as total 
			FROM new_applicant 
	...
	function getDataApplicantPositionSummaryRejected()
	{
		$sql = "
			SELECT position_applied, COUNT(*) as total 
			FROM new_applicant 
			WHERE deletests = 0 AND st_data = 3 
			GROUP BY position_applied
		";
	...
	function getDataNewApplicent($search = "", $page = 1)
	{
		...
		foreach ($rsl as $val)
		{
			...
			$btnAct .= "<button class=\"btn btn-danger btn-xs btn-block\" style=\"margin-top:5px;\" onclick=\"Reject('".$val->id."', '<b><i>:: ".$val->fullname." ::</i></b>');\">
				<i class=\"fas fa-ban\"></i> Reject Crew
			</button>";

			$btnAct .= "<button class=\"btn btn-success btn-xs btn-block\" style=\"margin-top:5px;\" onclick=\"draftCrew('".$val->id."', '<b><i>:: ".$val->fullname." ::</i></b>');\">
	...
	function getDataDraftCrew($search = "", $page = 1)
	{
		...
		foreach ($rsl as $val)
		{
			...
			$trListDataDraftCrew .= "<td>$val->fullname</td>";
	...
	function getDataRejectedCrew($search = "", $page = 1)
	{
		$page = isset($_GET['page']) && is_numeric($_GET['page']) ? intval($_GET['page']) : 1;
		$limit = 10;
		$offset = ($page - 1) * $limit;

		$trListDataRejectCrew = "";
		$no = $offset + 1;
		...
		foreach ($rsl as $val)
		{
			...
			$btnAct .= "<button class=\"btn btn-primary btn-xs btn-block\" style=\"margin-top:5px;\" onclick=\"unReject('".$val->id."');\">Un Reject</button>";


			$trListDataRejectCrew .= "<tr id='row_$val->id'>";
			$trListDataRejectCrew .= "<td class='text-center'>$no</td>";
			$trListDataRejectCrew .= "<td>$val->email</td>";
			$trListDataRejectCrew .= "<td>$val->fullname</td>";
			$trListDataRejectCrew .= "<td>$val->born_place</td>";
			$trListDataRejectCrew .= "<td>$val->born_date</td>";
			$trListDataRejectCrew .= "<td>$val->handphone</td>";
			$trListDataRejectCrew .= "<td class='position-applied'>$val->position_applied</td>";
			$trListDataRejectCrew .= "<td>$val->ijazah_terakhir</td>";
			$trListDataRejectCrew .= "<td>$val->last_experience</td>";
			$trListDataRejectCrew .= "<td>$val->pengalaman_jeniskapal</td>";
			$trListDataRejectCrew .= "<td>$val->berlayardengancrewasing</td>";
			$trListDataRejectCrew .= "<td>$val->last_salary</td>";
			$trListDataRejectCrew .= "<td>$val->join_inAndhika</td>";
			$trListDataRejectCrew .= "<td>$val->new_cv</td>";
			$trListDataRejectCrew .= "<td class='text-center'>$btnAct</td>";
			$trListDataRejectCrew .= "</tr>";
		...
		if ($page > 1) {
			$prevPage = $page - 1;
			$pagination .= "<button class='btn btn-sm btn-info' style='margin: 0 5px;' onclick='loadPageDataRejectedCrew($prevPage)'>⟨ Sebelumnya</button>";
		}
		...
		if ($page < $totalPages) {
			$nextPage = $page + 1;
			$pagination .= "<button class='btn btn-sm btn-info' style='margin: 0 5px;' onclick='loadPageDataRejectedCrew($nextPage)'>Selanjutnya ⟩</button>";
		}

		$pagination .= '</td></tr>';

		echo $trListDataRejectCrew . $pagination . $infoTotalData;
	}

	function setDraftCrew()
	{
	...
	function setDataRejectDraftCrew() 
	{
		$id = $this->input->post('id');
		...
		if ($id) {
			...
			echo json_encode(array(
				'status' => 'success',
				'message' => 'Data berhasil disimpan sebagai rejected draft crew.'
			));	
		} else {
			echo json_encode(array(
				'status' => 'error',
	...
	function setRejectCrew() {
		...
		if ($id) {
			...
			echo json_encode(array('status' => 'success', 'message' => 'Crew berhasil ditolak.'));
		} else {
			echo json_encode(array('status' => 'error', 'message' => 'ID tidak valid'));
		}
	}
