<script type="text/javascript">
	...
	function saveDataRegACC()
	{
		...
		var kdLicense = $("#slcLicenseCC").val();
		...
		$("#idLoadingForm").show();
		$.post('<?php echo base_url("compliance/saveDataRegD"); ?>',
		{ idEdit : idEdit,idPerson : idPerson,kdLicense : kdLicense,kdRank : kdRank,descCertCC : descCertCC,countryIssueCC : countryIssueCC,dateIssueCC : dateIssueCC,dateExpCC : dateExpCC,numberCC : numberCC,placeCC : placeCC,issueAuthCC : issueAuthCC },
			function(data)
			{
				alert(data);
				$("#idLoading").hide();
	...
	function getDataEditRegACC(id)
	{
		...
		{ id : id,type : 'editRegD',idPerson : idPerson },
			function(data)
			{
				$("#txtIdEditCC").val(data.idrg1);
				$("#slcLicenseCC").val(data.rg1license);
	...
	function delDataRegACC(id,idPerson)
	{
		...
		if(cfm)
		{
			...
			{ id : id,type : 'delRegD',idPerson : idPerson },
				function(data) 
				{
					alert(data);
					certRegD();
	...
	<div class="col-md-12 col-xs-12">
		<div class="row">
			<div class="col-md-12 col-xs-12" style="text-align:right;">
				<legend style="margin-bottom:10px;font-size: 18px;"><b><i>:: (D) Reg II / 1-4, III / 1-4 Officers Certificate of Competency & Ratings Watch-keeping Certificate (including flag state endorsements) ::</i></b></legend>
			</div>
		</div>
		<div class="row">
			...
			<div class="col-md-2 col-xs-12">
				<label for="slcLicenseCC">License :</label>
				<select class="form-control input-sm" id="slcLicenseCC">
					<option value=""> - </option>
		...
		<div class="row">			
			...
			<div class="col-md-1 col-xs-12">
				<label>&nbsp</label>
				<button class="btn btn-danger btn-sm btn-block" onclick="certRegD();" title="Cancel"><i class="glyphicon glyphicon-ban-circle"></i> Cancel</button>
			</div>
		</div>
	</div>
</div>
<div class="row" style="margin-top:10px;display:;" id="idDataTableCC">
	<div class="col-md-12 col-xs-12">
		<div class="row">
			...
			<div class="col-md-1 col-xs-12">
				<button class="btn btn-success btn-xs btn-block" title="Refresh" onclick="certRegD();"><i class="fa fa-refresh"></i> Refresh</button>
			</div>
			<div class="col-md-10 col-xs-12" style="text-align:right;">
				<legend style="margin-bottom:10px;font-size:16px;"><b><i>:: (D) Reg II / 1-4, III / 1-4 Officers Certificate of Competency & Ratings Watch-keeping Certificate (including flag state endorsements) ::</i></b></legend>
			</div>
		</div>
		<div class="row">
			<div class="col-md-12 col-xs-12">
