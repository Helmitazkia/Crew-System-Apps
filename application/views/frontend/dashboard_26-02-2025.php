...
<head>
                        ...
                        bar: {
                            ...
                            events: {
                                click: function(event) {
                                    ...
                                    if (!shipData.crew_names || !shipData
                                        .crew_ranks) {
                                        shipData.crew_names = [];
                                        shipData.crew_ranks = [];
                                    }
                                    ...
                                    if (selectedCategory === 'Total Crew') {
                                        crewList = shipData.crew_names.map((name,
                                                i) =>
                                            ({
                                                name: name,
                                                rank: shipData.crew_ranks[
                                                        i] ||
                                                    '-'
                                            }));
                                    ...
                                    modalBody.append(
                                        ...
                                        `).join('') :
                                        '<tr><td colspan="4" style="text-align: center;">No Crew Data Available</td></tr>'
                                    );
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                var seriesData = rankCategories.map(rank => ({
                    name: rank,
                    data: months.map(month => (rankSummary[month] && rankSummary[
                            month][
                            rank
                        ]) ?
                ...
                var totalData = months.map(month => {
                    return Object.values(rankSummary[month] || {}).reduce((sum,
                            value) =>
                        sum + value, 0);
                });
                        ...
                        column: {
                            ...
                            events: {
                                click: function(event) {
                                    ...
                                    $('#modalTitleCrewDetailByEstimatedSignOff')
                                        .text(
                                            `Crew Distribution for ${selectedMonth}`
                                        );
                                    $('#idBodyModalCrewDetailByEstimatedSignOff')
                                        .html(
                                            modalBody);
                ...
                $(document).on('click', '#convertToTableBtn', function() {
                    ...
                    var rankKeys = Object.keys(groupedData).sort((a, b) => (rankOrder[
                            a] ||
                        999) - (rankOrder[b] || 999));
                    var currentPage = 0;
    ...
    $(document).ready(function() {
        $.ajax({
            ...
            dataType: 'json',
            success: function(data) {
                var totalCrewOnboard = data.reduce((sum, item) => sum + item
                    .jumlah_crew_onboard,
                    0);
                ...
                Highcharts.chart('idTotalCrewByKapal', {
                            ...
                            point: {
                                events: {
                                    click: function() {
                                        ...
                                        var statusColor = totalCrew >= 22 ?
                                            "green" :
                                            "red";

                                        $("#idLblVesselStatus").text(
                                            `Status: ${vesselStatus}`).css(
                                            "color",
                                            statusColor);
                    ...
                    legend: {
                        ...
                        itemStyle: {
                            fontSize: '14px', // Adjust the font size for better visibility
                            fontWeight: 'bold' // Bold font for legend items
                        },
                        itemMarginTop: 5, // Add space between legend items and chart
                        itemMarginBottom: 5, // Add space below legend items
                        symbolPadding: 10 // Increase space between legend symbol and text
                    },
                    credits: {
                        enabled: false
                    },
...
<body style="background-color:#B3E0DC;">
    <div class="container">
