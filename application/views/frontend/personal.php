...
<head>
    ...
    $(document).ready(function() {
        $('#txtPercentageHomeSalary').on('input', function() {
            let homeVal = parseFloat($(this).val());

            if (isNaN(homeVal) || homeVal < 0) homeVal = 0;
            if (homeVal > 100) homeVal = 100;

            $('#txtPercentageHomeSalary').val(homeVal);

            if (homeVal === 0) {
                $('#txtPercentageBoardSalary').val(100);
            } else {
                const boardVal = 100 - homeVal;
                $('#txtPercentageBoardSalary').val(boardVal);
            }
        });

        $('#txtPercentageBoardSalary').on('input', function() {
            let boardVal = parseFloat($(this).val());

            if (isNaN(boardVal) || boardVal < 0) boardVal = 0;
            if (boardVal > 100) boardVal = 100;

            $('#txtPercentageBoardSalary').val(boardVal);

            if (boardVal === 0) {
                $('#txtPercentageHomeSalary').val(100);
            } else {
                const homeVal = 100 - boardVal;
                $('#txtPercentageHomeSalary').val(homeVal);
            }
        });
    });
    ...
    function saveDataPersonal() {
        ...
        var slcTaxStatus = $("#slcTaxStatus").val();
        var socialSecurityNo = $("#txtSosSecNumber").val();
        ...
        if (slcTaxStatus == "") {
            alert("Tax Status must be selected..!!");
            $("#slcTaxStatus").focus();
            return false;
        }
        if (socialSecurityNo == "") {
            alert("Social Security Must Be Filled..!!");
            $("#txtSosSecNumber").focus();
            return false;
        }
        if (bankName == "") {
        ...
        if (accNo == "") {
            alert("Account No Must Be Filled..!!");
            $("#txtAccNo").focus();
            return false;
        }
        if (accName == "") {
        ...
        formData.append('seafarerCode', $("#txtSeafarerCode").val());
        ...
        formData.append('taxStatus', $("#slcTaxStatus").val());
        ...
        formData.append('txtoldcontno', $("#txtoldcontno").val());
        formData.append('txtoldcrewID', $("#txtoldcrewID").val());
        ...
        formData.append('txtPercentageHomeSalary', $("#txtPercentageHomeSalary").val());
        formData.append('txtAccountName', $("#txtAccountName").val());
        formData.append('txtBankNameBoat', $("#txtBankNameBoat").val());
        formData.append('txtAccNoBoat', $("#txtAccNoBoat").val());
        formData.append('txtAccountNameBoat', $("#txtAccountNameBoat").val());
        formData.append('txtPercentageBoardSalary', $("#txtPercentageBoardSalary").val());
    ...
    function navProsesCrew() {
        ...
        if (slcNav == "personal_data") {
            ...
            $("#idFormBodyOther").load('<?php echo base_url("compliance/getData"); ?>' + '/' + idPerson,
                function() {
                    $("#idLoadingForm").hide();
                });
        } else if (slcNav == "sea") {
            ...
            $("#idFormBodyOther").load('<?php echo base_url("seaExperiance/getData"); ?>' + '/' + idPerson,
                function() {
                    $("#idLoadingForm").hide();
                    $("[id^=txtDate]").datepicker({
                        dateFormat: 'yy-mm-dd',
    ...
    function backButton() {
        ...
        elTextHide.forEach(el => el.value = '');
        elInput.forEach(el => el.value = '');
        elSlc.forEach(el => el.value = '');
        elUplFile.forEach(el => el.value = '');
        elTextArea.forEach(el => el.value = '');
        elChecked.forEach(el => el.checked = false);
        elRadio.forEach(el => el.checked = false);
    ...
    $(document).ready(function() {
        $('#txtAccNo, #txtAccNoBoat, #txtSosSecNumber, #txtTaxNumber').on('input', function() {
            this.value = this.value.replace(/[^0-9]/g, '');
        });
        $('#txtAccNo, #txtAccNoBoat, #txtSosSecNumber, #txtTaxNumber').on('blur', function() {
            if (this.value.trim() === '') {
                ...
                <div class="col-md-5 col-xs-12">
                    <div class="btn-group btn-group-justified" role="group" aria-label="Status Crew">
                        <div class="btn-group" role="group">
                            <button type="button" class="btn btn-pupple btn-xs" title="Data Pickup"
                                style="font-weight:bold;" onclick="navButtonHead('newapplicent');">Data Pickup</button>
                        </div>
                        <div class="btn-group" role="group">
                ...
                <div class="row">
                    <div class="col-md-6 col-xs-12">
                        <div class="row" style="margin-top:5px;">
                            ...
                            <div class="col-md-4 col-xs-12">
                                <label for="txtoldcontno" style="font-size:12px;">Old Contract Number:</label>
                                <input type="text" class="form-control input-sm" id="txtoldcontno" readonly>
                            </div>
                            <div class="col-md-4 col-xs-12">
                                <label for="txtoldcrewID" style="font-size:12px;">Old Crew ID:</label>
                                <input type="text" class="form-control input-sm" id="txtoldcrewID"
                                    placeholder="Old Crew ID" readonly>
                            </div>
                        </div>
                        <div class="row" style="margin-top:5px;">
                            <div class="col-md-4 col-xs-12">
                                <label for="txtSeafarerCode" style="font-size:12px;">Seafarer Code :</label>
                                <input type="text" class="form-control input-sm" id="txtSeafarerCode"
                        ...
                        <div class="row" style="margin-top:5px;">
                            ...
                            <div class="col-md-4 col-xs-12">
                                <label for="slcTaxStatus" style="font-size:12px;">Tax Status :</label>
                                <select class="form-control input-sm" id="slcTaxStatus" name="taxStatus">
                                    <?php echo $optTax; ?>
                                </select>
                            </div>
                        ...
                        <div class="row" style="margin-top:5px;">
                            <div class="col-md-12 col-xs-12">
                                <legend style="font-size:16px;float:right;margin-bottom:0px;margin-top:10px;">Home
                                    Salary:</legend>
                                <div class="row">
                                    ...
                                    <div class="col-md-3 col-xs-12">
                                        <label for="txtPercentageHomeSalary" style="font-size:12px;">Percentage Home
                                            Salary
                                            :</label>
                                        <input type="number" min="0" max="100" class="form-control input-sm"
                                            id="txtPercentageHomeSalary" placeholder="Percentage Home Salary">
                        ...
                        <div class="row" style="margin-top:5px;">
                            <div class="col-md-12 col-xs-12">
                                <legend style="font-size:16px;float:right;margin-bottom:0px;margin-top:10px;">Board
                                    Salary:</legend>
                                <div class="row">
                                    <div class="col-md-3 col-xs-12">
                                        <label for="txtBankNameBoat" style="font-size:12px;">Bank Name Board:</label>
                                        <input type="text" class="form-control input-sm" id="txtBankNameBoat"
                                            placeholder="Bank Name Board">
                                    </div>
                                    <div class="col-md-3 col-xs-12">
                                        <label for="txtAccNoBoat" style="font-size:12px;">Account Number Board:</label>
                                        <input type="text" class="form-control input-sm" id="txtAccNoBoat"
                                            placeholder="Account No Board">
                                    </div>
                                    <div class="col-md-3 col-xs-12">
                                        <label for="txtAccountNameBoat" style="font-size:12px;">Account Name
                                            Board:</label>
                                        <input type="text" class="form-control input-sm" id="txtAccountNameBoat"
                                            placeholder="Account Name Board">
                                    </div>
                                    <div class="col-md-3 col-xs-12">
                                        <label for="txtPercentageBoardSalary" style="font-size:12px;">Percentage Board
                                            Salary:</label>
                                        <input type="text" class="form-control input-sm" id="txtPercentageBoardSalary"
                                            placeholder="Percentage Board Salary">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-md-6 col-xs-12">
                        ...
                        <div class="row" style="margin-top:5px;">
                            <div class="col-md-3 col-xs-12">
                                <label for="chkNewApp" style="font-size:12px;">New applicant :</label><br>
                                <label class="checkbox-inline">
                                    <input type="checkbox" id="chkNewApp" value="1"> New applicant
                                </label>
                            </div>
                            <div class="col-md-3 col-xs-12">
                            ...
                            <div class="col-md-3 col-xs-12">
                                <label for="chkBlackList" style="font-size:12px;">Not for Employed :</label><br>
                                <label class="checkbox-inline">
                                    <input type="checkbox" id="chkBlackList" value="1"> Not for Employed
                                </label>
                            </div>
                            <div class="col-md-3 col-xs-12">
